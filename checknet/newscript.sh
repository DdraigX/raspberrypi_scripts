...
while true; do
	...
	do
		...
		if [ "$?" != "0" ]; then
			echo "$(date) : Failed to Ping $external_ip" >> /home/pi/netlog.log
			outage="TRUE"
			echo $i
		else
			echo "$(date) : Ping success : $external_ip" >> /home/pi/netlog.log
	...
	if [ "$outage" == "TRUE" ]; then
		...
		do
			...
			sleep 30s
			if [ "$?" != "0" ]; then 
				echo "$(date) : Internal IP down." >> /home/pi/netlog.log
	...
	# The final decision
	if [ "$outage" == "TRUE" ]; then
		echo "shutdown initiated" >> /home/pi/netlog.log
	fi
done
